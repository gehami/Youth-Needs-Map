...
########
full_spdf = readRDS('Metrics without time dependency.rds')
...
#pallette 
gang_presence_pal = colorNumeric('YlOrRd', domain = fin_metrics_spdf@data$gang_presence)
vocational_pal = colorNumeric('YlOrRd', domain = fin_metrics_spdf@data$vocational_training)
parent_pal = colorNumeric('YlOrRd', domain = fin_metrics_spdf@data$parent_awareness)

#initial map
map <- leaflet(fin_metrics_spdf) %>% 
  # add ocean basemap
...
gang_presence_map <- map %>% addPolygons(weight = 1, opacity = 1, color = 'white', dashArray = '3',
                                 fillColor = ~gang_presence_pal(gang_presence),
                                 fillOpacity = .8) %>%
  addLegend(pal = gang_presence_pal, values = fin_metrics_spdf$gang_presence, opacity = 0.7, position = 'bottomright') 



vocational_map <- map %>% addPolygons(weight = 1, opacity = 1, color = 'white', dashArray = '3',
                                     fillColor = ~vocational_pal(vocational_training),
                                     fillOpacity = .8) %>%
  addLegend(pal = vocational_pal, values = fin_metrics_spdf$vocational_training, opacity = 0.7, position = 'bottomright')


parent_awareness_map <- map %>% addPolygons(weight = 1, opacity = 1, color = 'white', dashArray = '3',
                                      fillColor = ~parent_pal(parent_awareness),
                                      fillOpacity = .8) %>%
  addLegend(pal = parent_pal, values = fin_metrics_spdf$parent_awareness, opacity = 0.7, position = 'bottomright')

## Adding hotspots
hotspots_17 = readRDS('hotspots 2017.rds')

comp_map <- gang_presence_map %>% addPolylines(data = hotspots_17)
